...
import atKeyword from './lib/tokens/atKeyword';
import string from './lib/tokens/string';
import separator from './lib/tokens/separator';
import comment, { multiLineComment } from './lib/token/comment';
...
export default function(source) {
	...
	while (!stream.eof()) {
		...
		if (token) {
			if (!accum && (token.type === 'whitespace' || token.type === 'comment')) {
				// Do not add formatting tokens at the beginning of token list
				continue;
			}
			...
			if (token.propertyTerminator) {
				ctx.add(createProperty(stream, tokens, token));
				tokens = [];
