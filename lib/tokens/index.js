...
import _interpolation from './interpolation';
...
export function value(stream) {
	return url(stream) || string(stream) || interpolation(stream) || number(stream)
		|| hash(stream) || keyword(stream) || important(stream) || operator(stream);
}
...
export function interpolation(stream) {
	return _interpolation(stream, any);
}
