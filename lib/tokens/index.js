...
export function any(stream) {
	return value(stream) || selector(stream) || separator(stream) || formatting(stream);
}

export function selector(stream) {
	return ident(stream) || atKeyword(stream) || className(stream) || id(stream)
		|| pseudo(stream) || attribute(stream) || combinator(stream);
}

export function value(stream) {
	return url(stream) || string(stream) || number(stream) || hash(stream)
		|| keyword(stream) || important(stream) || operator(stream);
}
